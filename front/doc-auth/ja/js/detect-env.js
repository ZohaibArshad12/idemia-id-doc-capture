...
function displayEnvironement (env) {
  ...
  if (env.envDetected) {
    ...
    if (!envOS.isSupported) {
      ...
      envDetectionPage.querySelector(descriptionClass).textContent = "サポートされていないオペレーティングシステムを使用しているようです。";
      browsersDescription.textContent = "より良い体験のために、以下のオペレーティングシステムのいずれかを使用してください";
      ...
      document.querySelector(stepCountrySelectionId).className = document.querySelector(stepCountrySelectionId).className.concat('d-none');
      envDetectionPage.className = envDetectionPage.className.replace('d-none', '');
      envDetectionPage.querySelector(descriptionClass).textContent = "サポートされていないブラウザを使用しているようです。";
      browsersDescription.textContent = "より良い体験のために以下のブラウザのいずれかを使用してください";
